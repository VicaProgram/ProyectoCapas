...
{
    public partial class PCom : Form //Declara form
    {
        ELocCom Ent = new ELocCom(); //Declara objeto
        NLocPro NegPro = new NLocPro(); //Declara objeto
        NLocReg NegReg = new NLocReg(); //Declara objeto
        public PCom()//inicializa
        {
            InitializeComponent();
        ...
        private void PCom_Load(object sender, EventArgs e) //Cuando carga
        {
        ...
        public void CarDat() //Declara metodo
        {
            try //Bloque try
            {
                ...
                List<ELocCom> Listar = new NLocCom().Listar();// Obtiene la lista de comunas.
                foreach (ELocCom item in Listar)// Itera sobre cada item obtenido.
                {
                    Grilla.Rows.Add(new object[] { "", item.IdCom, item.Nombre, item.IdPro, item.Pro.Nombre, item.IdReg, item.Reg.Nombre });// Agrega una fila en la grilla.
                }
            }
            catch (Exception ex)
            {
        ...
        public void Validar() //Declara metodo
        {
            ...
            {
                if ((TextIngMod.Text == textConCom.Text) && (ComboIngModPro.Text == textConPro.Text))  // Verifica si los campos coinciden.
                {
        ...
        private void Grilla_DoubleClick(object sender, EventArgs e)//Evento
        {
            ...
            textConCom.Text = this.Grilla.CurrentRow.Cells[2].Value.ToString();// Carga los datos
            TextIngMod.Text = this.Grilla.CurrentRow.Cells[2].Value.ToString();// Carga los datos
            textConPro.Text = this.Grilla.CurrentRow.Cells[4].Value.ToString();// Carga los datos
            ComboIngModPro.Text = this.Grilla.CurrentRow.Cells[4].Value.ToString();// Carga los datos
            ComboIngModReg.Text = this.Grilla.CurrentRow.Cells[6].Value.ToString();// Carga los datos
            TextIngMod.Enabled = false;// Deshabilita el campo
            ComboIngModPro.Enabled = false;// Deshabilita el combo
        }
        ...
        private void CheckIng_CheckedChanged(object sender, EventArgs e)//Evento
        {
            if (CheckIng.CheckState == CheckState.Checked)// Si la casilla está marcada
            {
                CheckMod.CheckState = CheckState.Unchecked;// Desmarca la casilla
                CheckEli.CheckState = CheckState.Unchecked;// Desmarca la casilla
                label7.Text = "Ingresar Comuna:";// Cambia el texto
                TextIngMod.Enabled = true;// Habilita el campo
                ComboIngModPro.Enabled = true;// Habilita el combo
        ...
        private void CheckMod_CheckedChanged(object sender, EventArgs e)//Evento
        {
            if (CheckMod.CheckState == CheckState.Checked)// Si la casilla está marcada
            {
                CheckIng.CheckState = CheckState.Unchecked;// Desmarca la casilla
                CheckEli.CheckState = CheckState.Unchecked;// Desmarca la casilla
                label7.Text = "Actualizar Comuna:";// Cambia el texto
                TextIngMod.Enabled = true;// Habilita el campo
                ComboIngModReg.Enabled = true;// Habilita el combo
                ComboIngModPro.Enabled = true;// Habilita el combo
        ...
        private void CheckEli_CheckedChanged(object sender, EventArgs e)//Evento
        {
            if (CheckEli.CheckState == CheckState.Checked)// Si la casilla está marcada
            {
                CheckIng.CheckState = CheckState.Unchecked;// Desmarca la casilla
                CheckMod.CheckState = CheckState.Unchecked;// Desmarca la casilla
                label7.Text = "Eliminar Comuna:";// Cambia el texto
        ...
        {
            int IdPro = Convert.ToInt32(ComboIngModReg.SelectedValue);//Obtiene el valor seleccionado del combo
            DataTable dt = NegPro.Filtrar(IdPro); //Llama al método
            ComboIngModPro.DisplayMember = "Nombre";// Establece la propiedad del combo
            ComboIngModPro.ValueMember = "IdPro";// Establece la propiedad del combo
            ComboIngModPro.DataSource = dt; //Llena el combo
        }

        private void ComboIngModPro_SelectedIndexChanged(object sender, EventArgs e)//Evento
        {
            Validar(); //Llama al metodo
        }
        ...
        private void TextIngMod_KeyPress(object sender, KeyPressEventArgs e)//Evento
        {
            if (char.IsDigit(e.KeyChar))//Verifica si la tecla presionada es un dígito
            {
                e.Handled = true; //Cancela la entrada del dígito
                MessageBox.Show("Solo se permiten letras.", "Sistema.", MessageBoxButtons.OK, MessageBoxIcon.Error); //Muestra un mensaje
            }
        }
        ...
        private void ButIng_Click(object sender, EventArgs e)//Evento
        {
            ...
            Ent.IdPro = Convert.ToInt32(ComboIngModPro.SelectedValue); //Obtiene el valor seleccionado del combo
            if (res == DialogResult.Yes) //Verifica si el usuario seleccionó "Sí" en el cuadro de diálogo.
            {
                Respuesta<bool> resultado = NLocCom.Ingresar(Ent); //Llama al método
        ...
        private void ButMod_Click(object sender, EventArgs e)//Evento
        {
            var res = MessageBox.Show("Esta seguro de la acción a realizar?", "Sistema.", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Warning); //Muestra un mensaje
            string Mensaje = string.Empty; //Declara una variable
            Ent.IdCom = Convert.ToInt32(textId.Text); //Convierte el texto ingresado
            Ent.Nombre = TextIngMod.Text;//Asigna el texto ingresado
            Ent.IdPro = Convert.ToInt32(ComboIngModPro.SelectedValue);//Obtiene el valor seleccionado del combo
            if (res == DialogResult.Yes)//Verifica si el usuario seleccionó "Sí" en el cuadro de diálogo.
            {
                Respuesta<bool> resultado = NLocCom.Actualizar(Ent);//Llama al método
                if (resultado.estado)//Verifica si el estado de la respuesta indica éxito
                {
        ...
        private void ButEli_Click(object sender, EventArgs e)//Evento
        {
            ...
            if (res == DialogResult.Yes)//Verifica si el usuario seleccionó "Sí" en el cuadro de diálogo.
            {
                ...
                {
                    Respuesta<bool> resultado = NLocCom.Eliminar(Id);//Llama al método
                    if (resultado.estado)//Verifica si el estado de la respuesta indica éxito
                    {
        ...
        private void ButLim_Click(object sender, EventArgs e)//Evento
        {
            ...
            textConCom.Clear();//Limpia el texto
            textConPro.Clear();//Limpia el texto
            TextIngMod.Enabled = false; //Desactiva el textbox
            ComboIngModPro.Enabled = false; //Desactiva el combo
