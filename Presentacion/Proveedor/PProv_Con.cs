...
{
    public partial class PProv_Con : Form //Declara Form
    {
        public PProv_Con() //Inicializa
        {
            InitializeComponent();
        ...
        private void PProv_Con_Load(object sender, EventArgs e)//Cuando carga
        {
        ...
        public void CarDat()//Declara metodo
        {
            try//Bloque try
            {
                ...
                List<EProv> Listar = new NProv().Listar();// Obtiene la lista.
                foreach (EProv item in Listar)// Itera sobre cada item obtenido.
                {
                    Grilla.Rows.Add(new object[] { "", item.IdProv, item.Nombre, item.Rut, item.IdCom, item.Com.Nombre, item.Direccion, item.Tel, item.Email, item.Giro, item.Descr });// Agrega una fila en la grilla.
                }
            }
            catch (Exception ex)
            {
        ...
        {
            PProv_Act pasar = new PProv_Act(); //Declara Objeto
            ...
            pasar.TextDesF.Text = this.Grilla.CurrentRow.Cells[10].Value.ToString(); //Le da el valor
            pasar.TextDesI.Text = this.Grilla.CurrentRow.Cells[10].Value.ToString(); //Le da el valor
        ...
        private void ButEli_Click(object sender, EventArgs e)//Evento
        {
            ...
            if (res == DialogResult.Yes)//Verifica si el usuario seleccionó "Sí" en el cuadro de diálogo.
            {
                ...
                {
                    Respuesta<bool> resultado = NProv.Eliminar(Id);//Llama al método
